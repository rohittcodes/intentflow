...
// DEPRECATED: Official Firecrawl preset removed to favor generic configuration
export const seedOfficialMCPs = mutation({
  args: {
    userId: v.string(),
  },
  handler: async () => {
    // No-op or return message that seeding is disabled
    return { message: "Official MCP seeding is disabled" };
  },
});

// Clean up non-Firecrawl official MCPs
// DEPRECATED: Cleanup disabled
