/**
 * LangGraph Integration for Intentflow
 *
 ...
 * Converts Intentflow workflows to LangGraph StateGraph
 */
export class LangGraphExecutor {
  ...
  private buildGraph() {
    ...
    for (const [sourceId, sourceEdges] of Array.from(this.edgesBySource.entries())) {
      ...
      if (sourceType === 'if-else' || sourceType === 'if / else' || sourceType === 'router') {
        if (!conditionalNodes.has(sourceId)) {
          ...
          // For router, if no route matches, we might want to end or have a default
          // If 'none' branch is defined in path map, use it. Otherwise it will stall or error.
          builder.addConditionalEdges(sourceId as any, routingFunction, pathMap as any);
  ...
  private async executeNodePure(
    ...
    switch (nodeType) {
      ...
      case 'arcade': {
        const result = await executeArcadeNode(node, state as WorkflowState, this.activeThreadId);
        return result;
      }
      case 'guardrails': {
        const result = await executeGuardrailsNode(node, state as WorkflowState, this.apiKeys);
        return result;
      }
      case 'mcp': {
        // Use generic MCP executor
        // Note: We might need to pass user secrets here if they are available in this.apiKeys
        // For now, we pass firecrawl key as legacy support, but really we should pass a secrets map
  ...
  private createConditionalRouter(nodeId: string) {
    return async (state: typeof WorkflowStateAnnotation.State) => {
      ...
      // For router nodes, it returns the matched route ID or 'none'
      return result.branch || (nodeType === 'router' ? 'none' : 'else');
    };
  }
  ...
  private async executeNode(node: WorkflowNode, state: WorkflowState): Promise<any> {
    ...
    switch (nodeType) {
      ...
      case 'script':
        ...
        const { executeCodeNode } = await import('./executors/code');
        return await executeCodeNode(node, state as WorkflowState, this.apiKeys);
   ...
   */
  async executeStream(input: any, config?: { threadId?: string; executionId?: string; recursionLimit?: number }) {
    const threadId = config?.threadId || `thread_${Date.now()}`;
    const recursionLimit = config?.recursionLimit || 25; // Default to 25 steps to prevent infinite loops
  ...
  async resumeFromAuth(threadId: string, resumeValue?: any, options?: { executionId?: string; recursionLimit?: number }) {
    this.activeThreadId = threadId;
   ...
   */
  async execute(input: any, config?: { threadId?: string; recursionLimit?: number }) {
    const threadId = config?.threadId || `thread_${Date.now()}`;
