...
export async function executeLogicNode(
  ...
  switch (type) {
    ...
    case 'router':
      return await executeRouterNode(data, state);
...
async function executeRouterNode(data: any, state: WorkflowState): Promise<any> {
  const routes = data.routes || [];
  console.log('Evaluating Router Node with routes:', routes.length);

  // Get input context
  const input = state.variables['lastOutput'] || state.variables['input'] || {};
  const lastOutput = state.variables['lastOutput'];

  for (const route of routes) {
    const conditionExpr = route.condition || 'false';
    try {
      const evalFunction = new Function('input', 'state', 'lastOutput', `return ${conditionExpr}`);
      const result = evalFunction(input, state, lastOutput);

      if (result) {
        console.log(`✅ Route matched: ${route.label} (${route.id})`);
        return {
          matched: true,
          routeId: route.id,
          label: route.label,
          branch: route.id // Return route ID as the branch identifier
        };
      }
    } catch (error) {
      console.error(`Error evaluating route ${route.label}:`, error);
    }
  }

  console.log('⚠️ No routes matched in Router Node');
  return {
    matched: false,
    branch: 'none'
  };
