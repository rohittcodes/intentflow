...
export default function PreviewPanel({ execution, nodeResults, isRunning, onClose }: PreviewPanelProps) {
  return (
    <AnimatePresence>
      ...
      >
        {/* Header */}
        <div className="p-20 border-b border-border sticky top-0 bg-accent-white">
          ...
          {/* Status Badge */}
          {execution && (
            <div className={`inline-flex items-center gap-8 px-12 py-6 rounded-md text-xs ${
              execution.status === 'running' ? 'bg-secondary text-primary' :
        ...
        <div className="p-20">
          {isRunning && Object.keys(nodeResults).length === 0 ? (
            <div className="text-center py-32">
              <svg className="w-48 h-48 mx-auto mb-16 text-primary animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
              </svg>
            ...
            <div className="space-y-16">
              {Object.entries(nodeResults).map(([nodeId, result]) => (
                <div key={nodeId} className="bg-background rounded-xl p-16 border border-border">
                  {/* Node Header */}
                  <div className="flex items-center justify-between mb-12">
                    <h3 className="text-label-medium text-foreground font-medium">
                      {nodeId}
                    </h3>
                    <span className={`text-xs px-8 py-4 rounded-6 ${
                      result.status === 'running' ? 'bg-secondary text-primary' :
                  ...
                  {result.output && (
                    <div className="mt-12">
                      ...
                      <div className="bg-accent-white rounded-md p-12 border border-border">
